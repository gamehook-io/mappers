...
const PARTY_BASE = 0xD16B;
const NICKNAME_BASE = 0xD2B5;
...
function firstLivingPartySlot() {
    for (let slot = 0; slot < PARTY_COUNT; slot++) {
        ...
        if (memory.wram.get_byte(hpAddress) !== 0x00 || memory.wram.get_byte(hpAddress + 1) !== 0x00) return slot;
    }
    return 0;
}

function preprocessor() {
    const battleHasStarted = memory.wram.get_byte(0xD057) !== 0x00 && memory.wram.get_byte(0xCCF5) !== 0x00;
    const battleIsEnding = memory.wram.get_byte(0xCCF6) === 0x01 || memory.wram.get_byte(0xCF0B) > 0x00;
