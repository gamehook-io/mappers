...
function stateFromRam() {
    ...
    if (memory.get_byte(0xD116) === 0) return 'Overworld';
    if (memory.get_byte(0xCBDB) === 1 || memory.get_byte(0xCFE9) > 0) return 'From Battle';
    if (memory.get_byte(PARTY_BASE) === memory.get_byte(0xCB0C)) return 'Battle';
    return previousState === 'Overworld' || previousState === 'To Battle' ? 'To Battle' : 'Battle';
}

function preprocessor() {
    const state = stateFromRam();
    const battleSlot = memory.get_byte(0xCFC6);
