...
const PARTY_BASE = 0xDA2A;
const NICKNAME_BASE = 0xDB8C;
...
function stateFromRam() {
    ...
    if (memory.wram.get_byte(0xD116) === 0) return 'Overworld';
    if (memory.wram.get_byte(0xCBDB) === 1 || memory.wram.get_byte(0xCFE9) > 0) return 'From Battle';
    if (memory.wram.get_byte(PARTY_BASE) === memory.wram.get_byte(0xCB0C)) return 'Battle';
    return previousState === 'Overworld' || previousState === 'To Battle' ? 'To Battle' : 'Battle';
}

function preprocessor() {
    const state = stateFromRam();
    const battleSlot = memory.wram.get_byte(0xCFC6);
